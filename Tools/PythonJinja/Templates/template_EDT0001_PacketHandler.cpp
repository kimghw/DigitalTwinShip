...
{% for table in tables%}
void EDT0001_{{table['table_name']}}_{{'%04d'|format(loop.index)}}_ToDb(nlohmann::json& jsonInput)
{
    ...
    {%- for column in table['columns'] %}
    {%- if not column.primarykey%}
    // {{'%02d'|format(loop.index-1)}}/{{'%02d'|format(table['columns']|length-1)}}. {{ column.name }} to Database
    //     Table name : {{table['table_name']}}, Column name : {{column.name}}, Type : {{column.type}}
    {%- if column.type == 'INT'%}
    int32 {{ column.name }} = 0;
    try {
